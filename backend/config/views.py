...
from services import (
    RakutenGoraServiceStub, GoogleMapsServiceStub, RakutenGoraServiceClient
)
...
class GolfSearchView(APIView):
    ...
    def post(self, request):
        ...
        # Choose implementation based on settings.USE_API_STUBS
        if getattr(settings, 'USE_API_STUBS', True):
            rakuten = RakutenGoraServiceStub()
        else:
            # Instantiate real Rakuten client (reads RAKUTEN_APPLICATION_ID from env)
            rakuten = RakutenGoraServiceClient()
